...
#include "usb_crc.h" // PATCHED: for get_time_us_32(), used to bound the RX wait
#include <stdint.h>
...
// PATCHED (see PATCHES.md): bounded. This clears the RX IRQ flags and waits for
// the write to take effect; if the RX state machine is desynced the flags can
// stay asserted, and the original unbounded spin then hangs inside the SOF
// interrupt. Every other wait in the host path is bounded for the same reason.
static __always_inline void pio_usb_bus_start_receive(const pio_port_t *pp) {
  ...
  while ((pp->pio_usb_rx->irq & IRQ_RX_ALL_MASK) != 0) {
    if ((uint32_t)(get_time_us_32() - start) > 2000) {
      break;
    }
  }
}
