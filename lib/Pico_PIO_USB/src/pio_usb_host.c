...
// PATCHED (see PATCHES.md): both flags are cleared only by the SOF timer
// callback, so if that callback is not running -- which is precisely the state
// these are used to recover from -- the original unbounded spins never return
// and take the caller's core with them. Give up after 50 ms; the caller is
// stopping or restarting the stack anyway, and a stuck flag is not a reason to
// hang the whole system.
#define PIO_USB_TIMER_FLAG_WAIT_US 50000

void pio_usb_host_stop(void) {
  cancel_timer_flag = true;
  uint32_t const start = get_time_us_32();
  while (cancel_timer_flag) {
    if ((uint32_t)(get_time_us_32() - start) > PIO_USB_TIMER_FLAG_WAIT_US) {
      cancel_timer_flag = false; // the timer is not going to answer
      break;
    }
  }
}
...
bool pio_usb_host_endpoint_abort_transfer(uint8_t root_idx, uint8_t device_address,
  ...
  //
  // PATCHED (see PATCHES.md): bounded. `transfer_started` is cleared only at the
  // end of the SOF timer callback, so if that callback is itself stuck inside a
  // PIO wait, this spins forever -- on core 1, inside tuh_task(), which is the
  // observed wedge. This is also the most likely caller to hit it, because
  // TinyUSB aborts endpoints exactly when a device detaches, i.e. the moment the
  // bus is most likely to have desynced.
  //
  // 10 frames is ten times the comment's own stated requirement. Past that the
  // timer is not coming back, and returning lets the caller tear the endpoint
  // down anyway.
  uint32_t const abort_start = get_time_us_32();
  while (ep->has_transfer && ep->transfer_started) {
    if ((uint32_t)(get_time_us_32() - abort_start) > 10000) {
      ep->transfer_started = false; // the SOF timer will not clear it
      break;
    }
    busy_wait_ms(1);
  }
