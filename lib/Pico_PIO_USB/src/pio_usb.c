...
// PATCHED (see lib/Pico_PIO_USB/PATCHES.md)
//
// Every wait on PIO hardware below was written as an unbounded spin. That is
// safe only while the state machines behave; when one desyncs -- which a device
// detaching mid-transfer can cause -- the flag never arrives and the loop never
// exits.
//
// These run inside the SOF alarm interrupt, so a hang there takes everything
// with it: the SOF callback never returns, so `transfer_started` is never
// cleared and `pio_usb_host_endpoint_abort_transfer()` spins forever waiting
// for it, and `cancel_timer_flag` is never cleared so `pio_usb_host_stop()`
// spins forever too. The visible symptom is core 1 frozen in USBHost.task()
// while core 0 carries on answering the PC perfectly -- which is exactly how
// this presented, and why it looked like a USB protocol problem for so long.
//
// A USB full-speed packet is at most ~1 ms on the wire, and these waits cover
// a single packet, so 2 ms is far beyond any legitimate case while still being
// a small fraction of the 3 s host watchdog. Bailing out leaves the transaction
// to fail and be retried, which is recoverable; spinning is not.
#define PIO_USB_TX_WAIT_US 2000

// Bumped whenever a wait above gives up. Nonzero means the bus desynced at
// least once -- exposed so the application can report it rather than having it
// pass unnoticed.
volatile uint32_t pio_usb_tx_timeouts = 0;

// Spin on `cond` until it holds or the deadline passes. Evaluates to true on
// timeout, so callers can abandon the transaction.
#define PIO_USB_WAIT_UNTIL(cond)                                               \
  ({                                                                           \
    uint32_t const _start = get_time_us_32();                                  \
    bool _timed_out = false;                                                   \
    while (!(cond)) {                                                          \
      if ((uint32_t)(get_time_us_32() - _start) > PIO_USB_TX_WAIT_US) {        \
        pio_usb_tx_timeouts++;                                                 \
        _timed_out = true;                                                     \
        break;                                                                 \
      }                                                                        \
...
static void __no_inline_not_in_flash_func(send_pre)(pio_port_t *pp) {
  ...
  PIO_USB_WAIT_UNTIL((pp->pio_usb_tx->irq & IRQ_TX_EOP_MASK) != 0); // PATCHED
  ...
  PIO_USB_WAIT_UNTIL(pp->pio_usb_tx->fdebug & stall_mask); // PATCHED
...
void __not_in_flash_func(pio_usb_bus_usb_transfer)(pio_port_t *pp,
  ...
  // PATCHED: bounded. If the SM never raises the completion IRQ, abandon the
  // packet rather than hanging the SOF interrupt forever.
  if (PIO_USB_WAIT_UNTIL((pp->pio_usb_tx->irq & IRQ_TX_ALL_MASK) != 0)) {
  ...
  if (pp->low_speed) {
    ...
    PIO_USB_WAIT_UNTIL(*pc > PIO_USB_TX_ENCODED_DATA_COMP);  // PATCHED
  } else {
    PIO_USB_WAIT_UNTIL(*pc >= PIO_USB_TX_ENCODED_DATA_COMP); // PATCHED
  }
}
